...
class AutomatedTepezzaApi:
    ...
    def get_data(self, every: int, filepath: str, start_from: int = 0) -> pandas.DataFrame:
        ...
        while True:
            if input(f"ready? {Colors.YELLOW}(y){Colors.ENDC} ") == "y":
                time.sleep(5)
                break
        ...
        for idx in range(start_from, len(self.zips), every):
            ...
            self._type_zipcode(keyboard, target_zip)
    ...
    @staticmethod
    def _tap(key: Union[str, Key], keyboard: KeyboardController, lower: int = 50, upper: int = 100) -> None:
        keyboard.press(key)
        time.sleep(random.uniform(lower / 1000, upper / 1000))
        keyboard.release(key)

    @classmethod
    def _type_zipcode(cls, keyboard: KeyboardController, zipcode: str) -> None:
        ...
        cls._tap(Key.backspace, keyboard)
