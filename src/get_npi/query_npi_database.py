...
if __name__ == '__main__':
    ...
    DF_SOURCE = 'data/processed/all_with_npi.csv'
    DF_DEST   = 'data/processed/all_with_npi.csv'
    OVERWRITE = False
    ...
    def get_queries(row: pandas.Series, min_: int, max_: int) -> DoctorQuery:
        ...
        query_params = {k: v for k, v in row.to_dict().items() if k in param_names}
    ...
    for df_index, row in df.iterrows():
        ...
        while True:
            try:
                ...
                try:
                    results = get_queries(row, idx, len(DROP_ORDER)).query_result_paged()
                ...
                if count == 0:
                    idx += 1
                    logger.debug(f'too restrictive, increasing floor to {DROP_ORDER[idx:]} (idx = {idx})')
                elif count == 1:
                    ...
                    idx -= 1
                    logger.debug(f'not restrictive enough, decreasing floor to {DROP_ORDER[idx:]} (idx = {idx})')
